...
float backgroundFrame = 0.0f;
float sleepFrame = 0.0f;

void Game::Update(float dt)
{
    if (this->State == GAME_SLEEP) {
        if (frameCount(dt, backgroundFrame, sleepForFrames)) {
    ...
    if (this->State == GAME_MENU) {
        if (start < 310 && frameCount(dt, sleepFrame, 0.065f)) {
            ++start;
        }
    }
    ...
    if (this->State == GAME_ACTIVE) {
        ...
        for (auto& Player : Players) {
            ...
            if (!Player->CollisionWith["block"] && !Player->CollisionWith["ladder"]) {
        ...
        bool isGameOver = !playersHaveLives();
    ...
    if (this->State == GAME_MENU) {
        ...
        if (this->Keys[GLFW_KEY_ENTER] && !this->KeysProcessed[GLFW_KEY_ENTER]) {
            ...
            if (this->Menu->Selected->Options[this->Menu->Selected->Selected]->Options.size() > 0) {
                ...
                auto& optionName = this->Menu->Selected->Options[this->Menu->Selected->Selected]->Name;
                auto& optionValue = this->Menu->Selected->Options[this->Menu->Selected->Selected]->Value;
                if (optionName == "GAME START") {
                    SoundEngine->stopAllSounds();
                    SoundEngine->play2D("../resources/audio/stage1.mp3", true);
                    this->State = GAME_ACTIVE;
                    this->Reset();
...
void Game::ResetPlayers(bool withPlayerLives)
{
    for (auto& Player : Players) {
...
void Game::Reset() {
    if (*PlayingPlayers > 1 && Players.size() < 2) {
        glm::vec3 PlayerVelocity(500.0f);
        glm::vec3 PlayerSize(50.0f, 50.0f, 1.0f);
        Players.push_back(new PlayerObject(glm::vec3(0.0f), PlayerSize, ResourceManager::GetTexture("character-init"), glm::vec3(1.0f), PlayerVelocity, 2));
    }
    if (*PlayingPlayers == 1 && Players.size() > 1) {
